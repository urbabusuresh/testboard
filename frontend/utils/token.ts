...
function isProjectDeveloper(projectRoles: ProjectRoleType[], projectId: number) {
  ...
  const developerRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'developer');
   const leadRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'lead');
    const leadApproverRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'leadApprover');
     const leadReviewerRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'leadReviewer');
  if (found.role === managerRoleIndex || found.role === developerRoleIndex || found.role === leadRoleIndex || found.role === leadApproverRoleIndex || found.role === leadReviewerRoleIndex) {
    return true;
  }
...
function isProjectLead(projectRoles: ProjectRoleType[], projectId: number) {
  if (!projectRoles) {
    return false;
  }
...
function isProjectLeadApprover(projectRoles: ProjectRoleType[], projectId: number) {
  if (!projectRoles) {
    return false;
  }
  ...
  const leadRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'leadApprover');
  if (found.role === leadRoleIndex ) {
    return true;
  }
...
function isProjectLeadReviewer(projectRoles: ProjectRoleType[], projectId: number) {
  if (!projectRoles) {
    return false;
  }
  ...
  const leadRoleIndex = memberRoles.findIndex((entry) => entry.uid === 'leadReviewer');
  if (found.role === leadRoleIndex ) {
    return true;
  }
...
export {
  ...
  isProjectReporter,
  isProjectLead,
  isProjectLeadApprover,
  isProjectLeadReviewer,
  isPrivatePath,
  checkSignInPage,
  fetchMyRoles,
};
