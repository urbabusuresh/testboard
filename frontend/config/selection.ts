...
import { TestTypeType,TestCaseIntent } from '@/types/testType';
import { PriorityType } from '@/types/priority';
import { LocaleType } from '@/types/locale';

const roles: GlobalRoleType[] = [{ uid: 'administrator' }, { uid: 'user' }];
const memberRoles: MemberRoleType[] = [{ uid: 'manager' }, { uid: 'developer' }, { uid: 'reporter' }, { uid: 'lead' }, { uid: 'leadReviewer' }, { uid: 'leadApprover' }, { uid: 'viewer' }, { uid: 'client' }];

const categoricalPalette = ['#fba91e', '#6ea56c', '#3ac6e1', '#feda2f', '#f15f47', '#244470', '#9c80bb', '#f595a6'];

const locales: LocaleType[] = [{ code: 'en', name: 'English' }];
...
const testCaseIntent: TestCaseIntent[]=[
{ uid: 'positive' },{ uid: 'negative' },];
