...
export default function BugzillaReportPage() {
  ...
  const fetchBugs = async (pageNum: number, f: Filters) => {
    setLoading(true);
    try {
      const res = await bugzillaApi.listBugs({
        page: pageNum,
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <header className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Bugzilla Dashboard</h1>
          <p className="text-gray-600 mt-2">Track, analyze and manage bug reports</p>
        </header>
          ...
          <CardBody>
            ...
            <div className="flex gap-3 mt-4">
              ...
              <Button 
                color="success" 
                variant="flat" 
                onPress={() => bugzillaApi.downloadBugsExcel(filters)}
                    ...
                    >
                      ...
                      >
                        {bugs.map((bug) => (
                          ...
                          >
                            <TableCell className="font-medium">{bug.bug_id}</TableCell>
                            <TableCell>
        ...
        >
          ...
          >
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  {selectedBug.comments && Array.isArray(selectedBug.comments) ? (
                    selectedBug.comments.map((c: any, idx: number) => (
                      <div key={idx} className="p-3 border rounded-md bg-gray-50">
                        <div className="text-xs text-gray-500">{c.who || c.author || ""} • {c.time || ""}</div>
                        <div className="mt-1 whitespace-pre-wrap">{c.text || c.comment || c}</div>
                      </div>
                    ))
                  ) : selectedBug.comments_noprivate ? (
                    <div className="p-3 border rounded-md bg-gray-50 whitespace-pre-wrap">{selectedBug.comments_noprivate}</div>
                  ) : selectedBug.comments_text ? (
                    <div className="p-3 border rounded-md bg-gray-50 whitespace-pre-wrap">{selectedBug.comments_text}</div>
                  ) : (
            ...
            <div className="p-4 border-t flex justify-end gap-2">
              ...
              <a href={getBugzillaUrl(selectedBug.bug_id)} target="_blank" rel="noreferrer">
                <Button color="primary">Open in Bugzilla</Button>
              </a>
            </div>
          </div>
        </div>
