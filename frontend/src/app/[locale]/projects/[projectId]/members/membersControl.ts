...
async function getProjectMember(projectId: number, role: string) {
  const fetchOptions = {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
  ...
  const url = `${apiServer}/members/getProjectMembers?projectId=${projectId}&roles=${role}`;
...
export { fetchProjectMembers, addMember, deleteMember, updateMember, getProjectMember};
